...
// Bump the limit BEFORE requiring the library
...
foreach ($themes as $key => $value) {
  ...
  [$status, $html, $err] = http_get('https://en.wikipedia.org/wiki/Vladimir_Putin');
  if ($status < 200 || $status >= 300 || !$html) {
    error_log("HTTP $status; cURL error: $err");
    die('Fetch failed.');
  }
...
//echo json_encode(['wiki-fragments_response_test_2' => $tab_strings_array]);
//print_r($tab_strings_array);
echo "After tab_strings_array print";
